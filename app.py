import requests
import pandas as pd
from ta.trend import MACD # <--- CHANGED: New library import
from ta.momentum import StochasticOscillator # <--- CHANGED: New library import
...
macd_instance = MACD(close=self.data['close'], window_fast=12, window_slow=26, window_sign=9, fillna=False) # <--- CHANGED: New ta logic
# Stochastic Oscillator (14, 3, 3) using ta.momentum.StochasticOscillator
stoch_instance = StochasticOscillator(high=self.data['high'], low=self.data['low'], close=self.data['close'], window=14, smooth_window=3, fillna=False) # <--- CHANGED: New ta logic

self.data['MACD_Hist'] = macd_instance.macd_diff() # <--- CHANGED: Direct column creation
self.data['Stoch_K'] = stoch_instance.stoch() # <--- CHANGED: Direct column creation
self.data['Stoch_D'] = stoch_instance.stoch_signal() # <--- CHANGED: Direct column creation

self.data.dropna(inplace=True)
# Lines 244-248 of original file removed (redundant renaming)
...
macd_instance = MACD(close=self.data['close'], window_fast=self.MACD_FAST, window_slow=self.MACD_SLOW, window_sign=self.MACD_SIGNAL, fillna=False) # <--- CHANGED: New ta logic
stoch_instance = StochasticOscillator(high=self.data['high'], low=self.data['low'], close=self.data['close'], window=self.STOCH_K, smooth_window=self.STOCH_D, fillna=False) # <--- CHANGED: New ta logic
...
# --- Robust Column Check ---
if not all(col in self.data.columns for col in ['MACD_Hist', 'Stoch_K', 'Stoch_D']): # <--- CHANGED: Simplified check
print("FATAL: One or more technical indicator columns were not created successfully. Invalidate data.")
self.data = pd.DataFrame()
return

# Lines 395-407 of original file removed/replaced (redundant renaming/robust check)
self.data.dropna(inplace=True)
...
macd_instance = MACD(close=temp_data['close'], window_fast=self.MACD_FAST, window_slow=self.MACD_SLOW, window_sign=self.MACD_SIGNAL, fillna=False) # <--- CHANGED: New ta logic
stoch_instance = StochasticOscillator(high=temp_data['high'], low=temp_data['low'], close=temp_data['close'], window=self.STOCH_K, smooth_window=self.STOCH_D, fillna=False) # <--- CHANGED: New ta logic
...
self.data['MACD_Hist'] = macd_instance.macd_diff() # <--- CHANGED: Direct column assignment
self.data['Stoch_K'] = stoch_instance.stoch() # <--- CHANGED: Direct column assignment
self.data['Stoch_D'] = stoch_instance.stoch_signal() # <--- CHANGED: Direct column assignment
